...
export default function PompentoebehorenCatalogPage() {
  ...
  // Load image mapping
  useEffect(() => {
    fetchJsonSafe('/data/pompentoebehoren_image_mapping.json')
      .then(data => {
        setImageMapping(data);
      })
      .catch(err => {
        console.error('Image mapping not found:', err);
      });
  }, []);

  useEffect(() => {
    fetchJsonSafe('/data/pompentoebehoren_grouped.json')
      .then(data => {
        // Apply image mapping
        const dataWithImages = data.map((group: any) => {
          const firstVariantSku = group.variants[0]?.sku;
          const groupImage = firstVariantSku ? imageMapping[firstVariantSku] : null;
          
          return {
            ...group,
            media: groupImage ? [{ url: groupImage.replace(/^\//, ''), role: 'main', type: 'image' }] : group.media,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-[#00ADEF] to-blue-500 text-white">
        <div className="container mx-auto px-4 py-12">
          <h1 className="text-4xl font-bold mb-4">🔩 Pompentoebehoren - Product Groups</h1>
